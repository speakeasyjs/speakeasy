...
//
// Generates a random key with the set A-Z a-z 0-9 and symbols (or
// completely random bytes), of any length
...
//        .random_bytes(=true)      generate key using random bytes
//                                  instead of ascii (more secure)
...
speakeasy.generate_key = function(options) {
  ...
  var random_bytes = true;
  var symbols = true;
  var mode = options.mode || 'totp';
  ...
  if (options.random_bytes !== undefined && options.random_bytes === false) {
    random_bytes = false;
  }
  ...
  SecretKey.params = {  algorithm      : 'sha1'
                      , length         : length
                      , name           : name
                      };
  if (!!options.issuer) SecretKey.params.issuer = options.issuer;
  if (mode === 'hotp') SecretKey.params.counter = counter; else SecretKey.params.step = step;
  SecretKey.url = qrURL;
...
// SecretKey.url([type[, name]])
//
// Generates an otpauth: url for the key -- cf., http://code.google.com/p/google-authenticator/wiki/KeyUriFormat
//   @type [ascii,hex,base32] defaults to base32
//   @name the name in the URL. defaults to SecretKey.name which defaults to 'Secret Key'
//   @mode [hotp,totp] defaults to 'totp'
//
// All arguments are optional
//
var qrURL = function(type, name, mode) {
  this.params = this.params || {};

  var key = this[type || 'base32'];
  if(!key) throw(new Error('invalid type'));

  name = String(name || this.params.name || '').trim().replace(/\s+/g, '');
  var issuer = this.params.issuer || '';
  if (issuer.length > 0) name = issuer + ':' + name;

  mode = (mode === 'hotp') ? 'hotp' : 'totp';

  var url = ['otpauth://', mode, '/', encodeURIComponent(name), '?secret=', encodeURIComponent(key)].join('');
  if (issuer.length > 0) url += '&issuer=' + encodeURIComponent(issuer);
  if ((!!this.params.algorithm) && (this.params.algorithm !== 'sha1')) url += '&algorithm=' + encodeURIComponent(this.params.algorithm);
  if ((!!this.params.length) && (this.params.length !== 6)) url += '&digits=' + encodeURIComponent(this.params.length);

  switch (mode) {
    case 'hotp':
      if (!!this.params.counter) url += '&counter=' + encodeURIComponent(this.params.counter);
      break;

//  case 'totp':
    default:
      if ((!!this.params.step) && (this.params.step !== 30)) url += '&period=' + encodeURIComponent(this.params.step);
      break;
  }
