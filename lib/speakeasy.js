...
//
// Generates an otpauth: url for the key.
...
//
function qrURL(type, name, mode) {
  name = String(name || this.name || '').trim().replace(/\s+/g, '');
  var key = this[type];
  if(!key) throw(new Error('invalid type'));
  mode = (mode === 'hotp') ? 'hotp' : 'totp';
  return ['otpauth://', mode, '/', encodeURIComponent(name), '?secret=', encodeURIComponent(key)].join('');
};
